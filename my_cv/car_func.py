...
def hist_image(img):
    assert img.ndim==2   #判断灰度图，若不是终止程序运行
...
def pre_process(orig_img):
    ...
    sobel_img = cv2.Sobel(blur_img, cv2.CV_16S, 1, 0, ksize=3)   #检测垂直边缘
    sobel_img = cv2.convertScaleAbs(sobel_img)    #图像增强，凸显边缘特征
...
def locate_carPlate(orig_img,pred_image,car_plate_w,car_plate_h):
    ...
    for i,contour in enumerate(contours):
        ...
        if verify_scale(rotate_rect):
            ret,rotate_rect2 = verify_color(rotate_rect,temp2_orig_img)  #漫水填充算法
            if ret == False:
                continue
            # 车牌位置矫正
...
def get_chars(car_plate,char_w,char_h):
...
def extract_char(car_plate,char_w,char_h):
    ...
    char_img_list = get_chars(binary_plate,char_w,char_h)
...
def cnn_select_carPlate(plate_list,model_path):
    ...
    with sess1.as_default():
        with sess1.graph.as_default():
            ...
            preds = tf.argmax(net_outs,1) #预测结果的索引号
            ...
            # 判断是不是车牌，是的话pred==1，并通过for循环排序选出概率最大的车牌
            result_index,result_prob = -1,0.
            for i,pred in enumerate(pred_list):
                if pred==1 and prob_list[i]>result_prob:
                    result_index,result_prob = i,prob_list[i]
            if result_index == -1:
                return False,plate_list[0]   #没有车牌
            else:
                return True,plate_list[result_index]   #输出概率最大的车牌
